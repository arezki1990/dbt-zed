...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --oracle-client-url) ORACLE_CLIENT=1; ORACLE_CLIENT_URL="$2"; shift 2 ;;
...
# --oracle-client: Oracle Instant Client (Basic Light) for the thick Oracle
# driver. Only a 10g / 11g server needs it; it is not redistributable, so
# it is fetched from Oracle under Oracle's own OTN licence and only when
# asked for. This runs first: a server that cannot get the client should
# say so before anything else is touched.
if [[ "$ORACLE_CLIENT" == 1 ]]; then
  echo "==> Oracle Instant Client (Basic Light) — Oracle's OTN licence applies"
  IC_ARCH=$(uname -m)
  if [[ -z "$ORACLE_CLIENT_URL" ]]; then
    case "$IC_ARCH" in
    ...
    ORACLE_CLIENT_URL="https://download.oracle.com/otn_software/linux/instantclient/$IC_ZIP"
  fi
  IC_ZIP=$(basename "$ORACLE_CLIENT_URL")
  ...
  mkdir -p /opt/oracle
  if ! curl -fsSL --retry 3 "$ORACLE_CLIENT_URL" -o "/tmp/$IC_ZIP"; then
    cat >&2 <<ICFAIL
could not download $ORACLE_CLIENT_URL
Oracle moves and gates these files, and a proxy may block them. Fetch
the Instant Client Basic Light zip for linux-$IC_ARCH by hand, unzip it
under /opt/oracle, and set ZDBT_EL_ORACLE_CLIENT_DIR to that directory in
...
ICFAIL
  ...
  ORACLE_LIB_DIR=$(find /opt/oracle -maxdepth 1 -type d -name 'instantclient_*' -newer /opt/oracle | sort | tail -1)
  [[ -n "$ORACLE_LIB_DIR" ]] || ORACLE_LIB_DIR=$(find /opt/oracle -maxdepth 1 -type d -name 'instantclient_*' | sort | tail -1)
...
# Where the thick driver finds Oracle Instant Client. The launcher sources
# this file and the systemd unit reads it as EnvironmentFile, so the worker
# the daemon spawns inherits it. A hand-written value is replaced, not
# appended to.
...
Installed and started. Next:
  1. /etc/zdbt-el-serve/env holds the token, the profile and the database URL
     slots — fill the URLs your el/connections.yml references. Oracle 12.1+
     needs no client software; a 10g / 11g server needs the thick driver:
     re-run with --oracle-client-url <Instant Client zip> (ORACLE.md).
